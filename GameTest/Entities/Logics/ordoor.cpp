...
OrDoor::OrDoor(b2World *world, string name, b2Vec2 pos) : Object(world, name, pos){
...
OrDoor* OrDoor::fromJsonData(b2World *world, Json::Value data, int index, string){
    string name = StringFormatter::format("ordoor_%i", index);
    if(data.isMember("name"))
        name = data["name"].asString();
    
    OrDoor *t = new OrDoor(world, name, b2Vec2(data["position"]["x"].asDouble(), data["position"]["y"].asDouble()));
...
void OrDoor::connect(Object *obj, string cmd){
    ...
    obj->destroySignal.connect(bind(&OrDoor::targetRemoved, this, _1));
