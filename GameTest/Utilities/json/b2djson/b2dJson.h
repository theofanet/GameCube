...
protected:
    ...
    std::map<std::string, b2Vec2> m_custom_vectors;
    Json::Value m_saved_world_value;
...
public:
    ...
    bool writeToFile(b2Body* body, const char* filename);
    ...
    void addCustomVector(std::string key, b2Vec2 vec);
    b2Vec2 loadCustomVector(std::string key);
    ...
    b2Body*  readBodyFromFile(const char* filename, std::string& errorMsg, b2World *existingWorld = NULL);
