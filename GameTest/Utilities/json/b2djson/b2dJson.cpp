...
std::string b2dJson::writeToString(b2World* world)
{
    ...
    Json::Value world_value = b2j(world);
    
    std::map<std::string, b2Vec2>::iterator pos;
    for (pos = m_custom_vectors.begin(); pos != m_custom_vectors.end(); ++pos)
        vecToJson(pos->first.c_str(), pos->second, world_value);
    
    Json::StyledWriter writer;
    return writer.write( world_value );
}
...
bool b2dJson::writeToFile(b2Body* body, const char* filename)
{
    if ( !body || !filename )
        return false;
...
Json::Value b2dJson::b2j(b2Fixture *fixture)
{
    ...
    if ( fixture->GetUserData() != NULL ){
        //int v = (int)fixture->GetUserData();
        //fixtureValue["userData"] = v;
    }
...
void b2dJson::addCustomVector(std::string key, b2Vec2 vec){
    m_custom_vectors[key] = vec;
}

b2Vec2 b2dJson::loadCustomVector(std::string key){
    return jsonToVec(key.c_str(), m_saved_world_value);
}
...
b2World* b2dJson::readFromString(std::string str, std::string& errorMsg, b2World* existingWorld)
{
    ...
    m_saved_world_value = worldValue;
...
b2Body* b2dJson::readBodyFromFile(const char* filename, std::string& errorMsg, b2World *existingWorld)
{
    if (!filename)
        return NULL;
    ...
    return j2b2Body(existingWorld, bodyValue);
}
...
b2Fixture* b2dJson::j2b2Fixture(b2Body* body, Json::Value &fixtureValue)
{
    ...
    //if(!fixtureValue["userData"].isNull())
    //    fixture->SetUserData((void *)fixtureValue.get("userData", false).asUInt());
...
float b2dJson::jsonToFloat(const char* name, Json::Value& value, int index, float defaultValue)
{
    ...
    if ( index > -1 ) {
        if ( value[name][index].isNull() )
            ...
            return value[name][index].asDouble();
    }
    else {
...
b2Vec2 b2dJson::jsonToVec(const char* name, Json::Value& value, int index, b2Vec2 defaultValue)
{
    ...
    if (index > -1) {
        ...
        if ( value[name]["x"][index].isInt() ) //usually 0 or 1
            ...
            vec.x = value[name]["x"][index].asDouble();
        ...
        if ( value[name]["y"][index].isInt() ) //usually 0 or 1
            ...
            vec.y = value[name]["y"][index].asDouble();
    }
    else {
